...
只需输入任何文本，gtr 会自动识别你想做什么：

```bash
gtr 113.901495,22.499501       # 识别为坐标 → 在WGS84/GCJ02/BD09之间转换
gtr 1727087511                 # 识别为时间戳 → 显示所有日期/时间格式
gtr https://api.example.com    # 识别为HTTP → 发送GET请求
gtr "hello world"              # 识别为文本 → 显示base64/url/md5转换
```
